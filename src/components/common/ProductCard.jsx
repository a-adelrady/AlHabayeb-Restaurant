...
export default function ProductCard({ product }) {
  ...
  const needsDetail = orderType === "sizes" || orderType === "quantity";
  ...
  const isLowStock =
    !isOutOfStock && product.stockQty !== undefined && product.stockQty < 10;
  ...
  const handleAdd = useCallback(
    (e) => {
      ...
      // لو المنتج عنده أحجام أو كمية → افتح صفحة التفاصيل
      if (needsDetail) {
    ...
    [addToCart, product, isOutOfStock, needsDetail, navigate],
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 sm:h-52 overflow-hidden bg-gray-200 dark:bg-zinc-800">
        ...
        {product.badge && (
          <span
            className={`absolute top-3 right-3 px-2.5 py-1 rounded-full text-xs font-bold ${product.badgeColor === "gold" ? "bg-gold-500 text-black" : "bg-red-500 text-white"}`}
          >
            {product.badge}
          </span>
        )}
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-1.5">
          <h3 className="font-bold dark:text-white text-gray-900 text-sm leading-tight flex-1">
            {product.name}
          </h3>
          <div
        ...
        <p className="text-xs dark:text-zinc-400 text-gray-500 leading-relaxed mb-3 line-clamp-2">
          {product.description}
        </p>
        ...
        <div className="flex items-center justify-between">
          ...
          {isOutOfStock ? (
            ...
            <div
              className="flex items-center bg-gold-500 rounded-xl overflow-hidden"
              role="group"
            ...
            >
              ...
              <span
                className="text-black font-bold text-sm w-6 text-center"
                aria-live="polite"
              >
                {cartItem.quantity}
              </span>
