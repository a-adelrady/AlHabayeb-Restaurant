...
import { MdSearch, MdExpandMore, MdExpandLess, MdWhatsapp, MdArchive, MdShoppingBag } from 'react-icons/md'
...
const NEXT_STATUS = { pending: 'preparing', preparing: 'on_the_way', on_the_way: 'delivered' }
...
export default function AdminOrders() {
  ...
  const filtered = useMemo(() => currentOrders.filter(o => {
  ...
  const buildContactUrl = (phone) => {
    ...
    const normalized = cleaned.startsWith('0') ? `20${cleaned.slice(1)}` : cleaned
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">{orders.length} نشط · {archivedOrders.length} أرشيف</p>
        </div>
      </div>
      ...
      <div className="flex gap-2" role="tablist">
        {[
          { id: 'active',   label: 'الطلبات النشطة', icon: MdShoppingBag, count: orders.length },
          { id: 'archived', label: 'الأرشيف',         icon: MdArchive,     count: archivedOrders.length },
        ].map(tab => (
          <button key={tab.id} role="tab" aria-selected={activeTab === tab.id}
            onClick={() => { setActiveTab(tab.id); setFilterSt('all'); setSearch('') }}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
              activeTab === tab.id ? 'bg-gold-500 text-black' : 'bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white'}`}>
            <tab.icon className="text-base" />
            {tab.label}
            <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-black/20' : 'bg-zinc-700 text-zinc-300'}`}>{tab.count}</span>
          </button>
        ))}
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col sm:flex-row gap-3">
        <div className="flex-1 relative">
          ...
          <input type="search" placeholder="بحث بالاسم / هاتف / رقم طلب..." value={search}
            onChange={e => setSearch(e.target.value)} aria-label="بحث في الطلبات"
        ...
        {activeTab === 'active' && (
          <div className="flex gap-2 flex-wrap">
            {STATUS_OPTS.map(opt => (
              <button key={opt.value} onClick={() => setFilterSt(opt.value)}
      ...
      {filtered.length === 0 ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl py-16 text-center">
          ...
          {search && (
            <button onClick={() => setSearch('')} className="text-gold-400 text-xs mt-2 hover:text-gold-300">مسح البحث</button>
          )}
        </div>
      ) : (
            ...
            return (
              <motion.div key={order.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div
                  role="button"
                  tabIndex={0}
                  onKeyDown={e => e.key === 'Enter' && setExpandedId(expanded ? null : order.id)}
                ...
                >
                  ...
                  <div className="text-right flex-shrink-0 mr-2">
                    <p className="font-bold text-gold-400 text-sm">{formatPrice(order.total)}</p>
                    <p className="text-zinc-600 text-xs">{order.items.length} صنف</p>
                  </div>
                  {expanded
                    ...
                    >
                      <div className="border-t border-zinc-800 p-4 space-y-4">
                        <div className="grid sm:grid-cols-2 gap-4">
                          {/* Customer info */}
                          <div>
                            <p className="text-zinc-500 text-xs mb-2 font-medium">بيانات العميل</p>
                            <div className="space-y-1">
                              <p className="text-white text-sm font-semibold">{order.customer.name}</p>
                              <p className="text-zinc-400 text-sm" dir="ltr">{order.customer.phone}</p>
                              <p className="text-zinc-400 text-sm">{order.customer.address}</p>
                              {order.zone && <p className="text-zinc-500 text-xs bg-zinc-800 px-2 py-1 rounded-lg inline-block">📍 {order.zone.name}</p>}
                              {order.customer.notes && <p className="text-zinc-500 text-xs bg-zinc-800 p-2 rounded-lg">ملاحظات: {order.customer.notes}</p>}
                            </div>
                          </div>
                          {/* Items */}
                          <div>
                            <p className="text-zinc-500 text-xs mb-2 font-medium">المنتجات</p>
                            <div className="space-y-1.5">
                              {order.items.map((item, i) => (
                                <div key={i} className="flex justify-between text-sm bg-zinc-800 px-3 py-2 rounded-lg">
                              ...
                              {order.deliveryFee > 0 && (
                                <div className="flex justify-between text-xs text-zinc-500 px-1">
                                  <span>رسوم التوصيل</span><span>{formatPrice(order.deliveryFee)}</span>
                                </div>
                              )}
                              <div className="flex justify-between text-sm pt-1 border-t border-zinc-700">
                                ...
                                <span className="font-bold text-gold-400">{formatPrice(order.total)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        {activeTab === 'active' && (
                          <div className="flex flex-wrap gap-2 pt-2 border-t border-zinc-800">
                            ...
                            <a href={buildContactUrl(order.customer.phone)}
