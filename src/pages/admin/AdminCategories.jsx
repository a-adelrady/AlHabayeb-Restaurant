...
export default function AdminCategories() {
  ...
  const confirmDelete = (cat) => {
    if (cat.id === "all") {
      toast.error("لا يمكن حذف تصنيف الكل");
      return;
    }
    const count = products.filter((p) => p.category === cat.id).length;
    if (count > 0) {
      toast.error(`لا يمكن الحذف — ${count} منتج في هذا التصنيف`);
      return;
    }
    setDeleteTarget(cat);
  };
          ...
          {deleteTarget && (
            ...
            >
              <p className="text-red-300 text-sm">
                حذف تصنيف <strong>{deleteTarget.name}</strong>؟
              </p>
                ...
                {cat.id !== "all" && (
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                    <button
                      ...
                      aria-label={`تعديل ${cat.name}`}
                      className="p-2 bg-zinc-800 hover:bg-gold-500/20 hover:text-gold-400 text-zinc-400 rounded-xl transition-all"
                    >
                    ...
                    <button
                      ...
                      className="p-2 bg-zinc-800 hover:bg-red-500/20 hover:text-red-400 text-zinc-400 rounded-xl transition-all"
                    >
