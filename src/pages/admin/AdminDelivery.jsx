...
import { MdAdd, MdEdit, MdDelete, MdClose, MdDeliveryDining, MdAccessTime, MdAttachMoney } from 'react-icons/md'
...
export default function AdminDelivery() {
  ...
  const openAdd  = () => { setForm(EMPTY_ZONE); setEditingId(null); setErrors({}); setShowModal(true) }
  const openEdit = (z) => { setForm({ ...z, fee: String(z.fee) }); setEditingId(z.id); setErrors({}); setShowModal(true) }

  const validate = () => {
    const e = {}
    if (!form.name.trim())     e.name = 'اسم المنطقة مطلوب'
    if (form.fee === '')       e.fee  = 'رسوم التوصيل مطلوبة'
    else if (isNaN(Number(form.fee)) || Number(form.fee) < 0) e.fee = 'أدخل رقماً صحيحاً'
    return e
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">إدارة مناطق التوصيل ورسومها</p>
        </div>
        <button onClick={openAdd} className="flex items-center gap-2 bg-gold-500 hover:bg-gold-600 text-black font-bold px-4 py-2.5 rounded-xl text-sm transition-all">
          <MdAdd className="text-xl" /><span className="hidden sm:inline">إضافة منطقة</span>
        </button>
      </div>
      ...
      <AnimatePresence>
        {deleteTarget && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <p className="text-red-300 text-sm">حذف منطقة <strong>{deleteTarget.name}</strong>؟ هذا الإجراء لا يمكن التراجع عنه.</p>
            <div className="flex gap-2 flex-shrink-0">
              ...
              <button onClick={() => handleDelete(deleteTarget.id)} className="px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-sm">حذف</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {deliveryZones.map(zone => (
          <motion.div key={zone.id} layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                <button onClick={() => openEdit(zone)} aria-label={`تعديل ${zone.name}`}
                  className="p-1.5 bg-zinc-800 hover:bg-gold-500/20 hover:text-gold-400 text-zinc-400 rounded-lg transition-all">
                  <MdEdit className="text-sm" />
                </button>
                <button onClick={() => setDeleteTarget(zone)} aria-label={`حذف ${zone.name}`}
            ...
            <h3 className="font-bold text-white text-base mb-3">{zone.name}</h3>

            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm">
                <MdAttachMoney className="text-gold-400 text-base flex-shrink-0" />
                <span className="text-zinc-400">رسوم التوصيل:</span>
                <span className={`font-bold ${zone.fee === 0 ? 'text-green-400' : 'text-white'}`}>
        ...
        {deliveryZones.length === 0 && (
          <div className="col-span-full bg-zinc-900 border border-zinc-800 rounded-2xl py-16 text-center">
            ...
            <p className="text-zinc-500 text-sm">لا توجد مناطق توصيل. أضف منطقة الآن.</p>
          </div>
        )}
      </div>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="w-full max-w-sm bg-zinc-900 border border-zinc-800 rounded-2xl pointer-events-auto"
                role="dialog" aria-modal="true" aria-label={editingId ? 'تعديل منطقة' : 'إضافة منطقة'}>
                <div className="border-b border-zinc-800 px-6 py-4 flex items-center justify-between">
                  <h2 className="font-bold text-white">{editingId ? 'تعديل منطقة' : 'إضافة منطقة'}</h2>
                  <button onClick={() => setShowModal(false)} aria-label="إغلاق"><MdClose className="text-zinc-400 text-xl" /></button>
                </div>
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  <div>
                    <label className="text-zinc-400 text-xs mb-1 block">اسم المنطقة *</label>
                  ...
                  <div>
                    <label className="text-zinc-400 text-xs mb-1 block">رسوم التوصيل (ج.م) *</label>
                    <input type="number" min="0" value={form.fee} onChange={e => { setForm(f => ({ ...f, fee: e.target.value })); setErrors(er => ({ ...er, fee: '' })) }}
                  ...
                  <div>
                    <label className="text-zinc-400 text-xs mb-1 block">وقت التوصيل المتوقع</label>
