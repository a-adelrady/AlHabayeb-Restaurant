...
export default function UserProfilePage() {
  ...
  const myOrders = [...orders, ...archivedOrders]
    .filter((o) => {
      ...
      // match بالـ uid لو موجود
      if (o.userUid && currentUser.uid) return o.userUid === currentUser.uid;
      // fallback بالـ phone
      if (o.customer?.phone && currentUser.phone) {
    ...
    return (
      <div className="min-h-screen dark:bg-zinc-950 bg-gray-50 pt-24 flex items-center justify-center px-4">
        <div className="text-center">
          <div className="text-6xl mb-4" role="img" aria-label="مستخدم">
            👤
          </div>
          <h2 className="text-xl font-bold dark:text-white text-gray-900 mb-3">
            سجّل دخولك أولاً
          </h2>
          <p className="dark:text-zinc-400 text-gray-500 mb-6 text-sm">
            لمشاهدة طلباتك وإدارة حسابك
          </p>
          <div className="flex gap-3 justify-center flex-wrap">
            <Link
              to="/auth"
              className="bg-gold-500 hover:bg-gold-600 text-black font-bold px-6 py-3 rounded-xl transition-all"
            >
              تسجيل الدخول
            </Link>
            <Link
        ...
        <div className="max-w-3xl mx-auto px-4 py-8 space-y-5">
          ...
          >
            <div className="flex items-start gap-4">
              <div
                className="w-16 h-16 rounded-2xl bg-gold-500/20 border-2 border-gold-500/30 flex items-center justify-center text-gold-400 text-2xl font-bold flex-shrink-0"
                aria-hidden="true"
              >
          ...
          >
            <h3 className="font-bold dark:text-white text-gray-900 text-lg mb-4 flex items-center gap-2">
              <MdShoppingBag className="text-gold-500" aria-hidden="true" />{" "}
              طلباتي
            </h3>

            {myOrders.length === 0 ? (
              <div className="dark:bg-zinc-900 bg-white rounded-2xl border dark:border-zinc-800 border-gray-100 py-14 text-center">
                <div className="text-5xl mb-3" role="img" aria-label="طعام">
                  🍽️
                </div>
                <p className="dark:text-zinc-400 text-gray-500 text-sm mb-4">
                  لم تطلب أي شيء بعد!
                </p>
                <Link
                  to="/menu"
                  className="bg-gold-500 hover:bg-gold-600 text-black font-bold px-6 py-2.5 rounded-xl text-sm inline-block transition-all"
                >
                  تصفح المنيو
                </Link>
              </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="space-y-0.5">
                            {order.items.slice(0, 2).map((item, j) => (
                              <p
                                key={j}
                                className="text-sm dark:text-zinc-300 text-gray-700 truncate"
                              >
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          >
                            تتبع{" "}
                            <MdArrowForward className="rotate-180 text-sm" />
                          </Link>
                        </div>
                      </div>
                    </motion.article>
