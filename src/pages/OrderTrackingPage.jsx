...
export default function OrderTrackingPage() {
  ...
  useEffect(() => {
    ...
    if (DEMO_MODE) { setLoading(false); setSearched(true); return; }
  ...
  useEffect(() => {
    ...
    const unsubOrders = onSnapshot(
      ...
      (snap) => {
        ...
        // لو اختفى من orders → ابحث في archivedOrders
        else {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="تتبع حالة طلبك من مطعم الحبايب في الوقت الفعلي" />
      </Helmet>

      <div className="min-h-screen dark:bg-zinc-950 bg-gray-50 pt-20">
        <div className="max-w-2xl mx-auto px-4 py-10">
          <div className="text-center mb-10">
            <h1 className="text-3xl font-bold dark:text-white text-gray-900 mb-2">تتبع طلبك</h1>
            <p className="dark:text-zinc-400 text-gray-500">أدخل رقم الطلب لمعرفة حالته</p>
          </div>

          <form onSubmit={handleSearch} role="search" className="flex gap-3 mb-8">
            <div className="flex-1">
              <label className="sr-only" htmlFor="order-search">رقم الطلب</label>
              <input
                id="order-search"
                type="text"
          ...
          {loading && (
            <div className="text-center py-16">
              ...
              <p className="dark:text-zinc-400 text-gray-500 text-sm">جاري البحث...</p>
            </div>
          )}
          ...
          {!loading && searched && !order && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
              ...
              <h3 className="text-xl font-bold dark:text-white text-gray-900 mb-2">الطلب غير موجود</h3>
              <p className="dark:text-zinc-400 text-gray-500 text-sm">تأكد من رقم الطلب وحاول مرة أخرى</p>
            </motion.div>
          )}
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
              ...
              <div className="dark:bg-zinc-900 bg-white rounded-2xl p-6 border dark:border-zinc-800 border-gray-100">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <p className="text-sm dark:text-zinc-400 text-gray-500 mb-1">رقم الطلب</p>
                    <p className="font-mono font-bold text-gold-400 text-sm">{order.id}</p>
                  </div>
                  <span className={`px-3 py-1.5 rounded-full text-sm font-bold ${statusInfo?.color} ${statusInfo?.bg}`}>
                    {statusInfo?.text}
                  </span>
                </div>
              ...
              {order.status !== "cancelled" ? (
                <div className="dark:bg-zinc-900 bg-white rounded-2xl p-6 border dark:border-zinc-800 border-gray-100">
                  <h3 className="font-bold dark:text-white text-gray-900 mb-6">مسار الطلب</h3>
                  <ol className="relative space-y-6">
                    <div className="absolute right-5 top-5 bottom-5 w-0.5 dark:bg-zinc-700 bg-gray-200" aria-hidden="true" />
                    <div className="absolute right-5 top-5 w-0.5 bg-gold-500 transition-all duration-1000"
                    ...
                    {STEPS.map((step, idx) => {
                      ...
                      return (
                        <li key={step.status} className="flex items-center gap-4 relative">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center z-10 transition-all duration-500 flex-shrink-0 text-lg
                            ${done ? "bg-gold-500" : "dark:bg-zinc-800 bg-gray-100"}
                            ${active ? "ring-4 ring-gold-500/30 scale-110" : ""}`}
                            aria-current={active ? "step" : undefined}>
                            {done ? (active ? step.icon : "✓") : <MdRadioButtonUnchecked className="text-zinc-500 text-lg" />}
                          </div>
                          <div>
                            <p className={`font-semibold text-sm ${done ? "dark:text-white text-gray-900" : "dark:text-zinc-500 text-gray-400"}`}>
                              {step.label}
                            </p>
                            {active && <p className="text-xs text-gold-500 mt-0.5 animate-pulse">جاري الآن...</p>}
                          </div>
                        </li>
                      );
                    })}
                ...
                <div className="dark:bg-zinc-900 bg-white rounded-2xl p-6 border border-red-500/30 text-center">
                  <div className="text-4xl mb-3" role="img" aria-label="ملغي">❌</div>
                  <p className="font-bold text-red-400 text-lg">تم إلغاء الطلب</p>
                </div>
              )}

              {/* Items */}
              <div className="dark:bg-zinc-900 bg-white rounded-2xl p-6 border dark:border-zinc-800 border-gray-100">
                <h3 className="font-bold dark:text-white text-gray-900 mb-4">تفاصيل الطلب</h3>
                <div className="space-y-2.5 mb-3">
                  {order.items.map((item, i) => (
                    <div key={i} className="flex justify-between text-sm">
                      <span className="dark:text-zinc-300 text-gray-700">{item.name} × {item.quantity}</span>
                      <span className="font-medium dark:text-white text-gray-900">{formatPrice(item.price * item.quantity)}</span>
                    </div>
                  ))}
                </div>
                {order.deliveryFee > 0 && (
                ...
                <div className="border-t dark:border-zinc-800 border-gray-200 pt-3 flex justify-between">
                  ...
                  <span className="font-bold text-gold-500">{formatPrice(order.total)}</span>
                </div>
              </div>
