...
export default function CartPage() {
  ...
  if (cartItems.length === 0)
    return (
      <>
        <Helmet>
          <title>السلة — مطعم الحبايب</title>
        </Helmet>
        <div className="min-h-screen dark:bg-zinc-950 bg-gray-50 pt-20 flex items-center justify-center px-4">
          ...
          >
            ...
            <p className="dark:text-zinc-400 text-gray-500 mb-8">
              لم تضف أي وجبات بعد
            </p>
            <Link
              to="/menu"
              className="bg-gold-500 hover:bg-gold-600 text-black font-bold px-8 py-3 rounded-xl inline-block transition-all"
            >
              تصفح المنيو
            </Link>
          </motion.div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold dark:text-white text-gray-900">
                السلة{" "}
                <span className="text-gold-500">({cartItems.length})</span>
              </h1>
            </div>
            <button
              onClick={() => {
          ...
          <div className="grid lg:grid-cols-3 gap-6">
              ...
              <AnimatePresence>
                {cartItems.map((item) => (
                  <motion.div
                    ...
                    className="dark:bg-zinc-900 bg-white rounded-2xl p-3 border dark:border-zinc-800 border-gray-100"
                  >
                    {/* Row 1 — صورة + معلومات + زرار حذف */}
                    <div className="flex gap-3 mb-3">
                      <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-gray-200 dark:bg-zinc-800">
                        <img
                          src={item.image}
                          alt={item.name}
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold dark:text-white text-gray-900 text-sm leading-tight line-clamp-2">
                          {item.name}
                        </h3>
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-red-400 hover:bg-red-500/10 transition-all flex-shrink-0 self-start"
                      >
                        <MdDelete className="text-lg" />
                      </button>
                    </div>

                    {/* Row 2 — الكمية + الإجمالي */}
                    <div className="flex items-center justify-between">
                      <div
                        className="flex items-center bg-gold-500 rounded-xl"
                        role="group"
                      ...
                      >
                        <button
                          ...
                          className="p-2 hover:bg-gold-600 transition-colors rounded-r-xl"
                        >
                          <MdRemove className="text-black text-sm" />
                        </button>
                        <span
                          className="text-black font-bold text-sm w-7 text-center"
                          aria-live="polite"
                        >
                          {item.quantity}
                        </span>
                        ...
                        <button
                          ...
                          className="p-2 hover:bg-gold-600 transition-colors rounded-l-xl"
                        >
                          <MdAdd className="text-black text-sm" />
                        </button>
                      </div>
                      <p className="text-xs dark:text-zinc-400 text-gray-500 font-medium">
                        الإجمالي:{" "}
            ...
            <div className="lg:col-span-1">
              <div className="dark:bg-zinc-900 bg-white rounded-2xl p-6 border dark:border-zinc-800 border-gray-100 sticky top-24">
                <h2 className="font-bold text-lg dark:text-white text-gray-900 mb-5">
                  ملخص الطلب
                </h2>
                <div className="space-y-2.5 mb-4 max-h-44 overflow-y-auto">
                  {cartItems.map((item) => (
                    ...
                    >
                      <span className="dark:text-zinc-400 text-gray-600 flex-1 truncate">
                        {item.name} × {item.quantity}
                      </span>
                      <span className="dark:text-white text-gray-900 font-medium ml-2 flex-shrink-0">
                        {formatPrice(item.price * item.quantity)}
                      </span>
                    </div>
                  ))}
                ...
                <Link
                  to="/menu"
                  className="w-full block text-center mt-3 dark:text-zinc-400 text-gray-500 hover:text-gold-500 text-sm transition-colors py-2"
                >
                  + إضافة المزيد
                </Link>
              </div>
